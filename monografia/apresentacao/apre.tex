...
\frame{
  \frametitle{Diferenças - Memória}
  O OpenCL cria um objeto chamado Memory Object para abstrair uma posição na memória do device. Já o CUDA usa ponteiros do C
  para referenciar posições de memória no kernel. Nas duas linguagens é necessário usar funções para iniciar e alocar o espaço
  na memoria do device e para passar a memória para essas posições. No CUDA ainda é necessário desalocar essas posições.
}
...
\frame{
  ...
  O modelo de execução representa o programa que roda no Host, e todos os kernels em execução num dado momento. 
  O host é responsável pela criação de um contexto, que será usado para troca de informações entre o host e o kernel. \\
  Ao iniciar a execução do kernel, é necessário determinar um espaço de indices para as threads, esse espaço tem dimensões
  variáveis de uma até três.
}
...
\frame{Explicação dos PTX}

\frame{Comparação dos PTX}
